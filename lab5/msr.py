...
class MultiStepBuffer:
    ...
    def _get_n_step_transition(self):
        ...
        for idx, (_, _, r, ns, d) in enumerate(self.buffer):
...
class DQNAgent:
    def __init__(self, env_name, args=None):
        ...
        self.multi_step_buffer = MultiStepBuffer(self.n_step, self.gamma)
    ...
    def run(self, episodes=2000):
        for ep in range(episodes):
            ...
            while not done and step_count < self.max_episode_steps:
                ...
                n_step_transition = self.multi_step_buffer.add((state, action, reward, next_state, done))
                if n_step_transition:
                    self.memory.add(n_step_transition)
                if done:
                    while len(self.multi_step_buffer.buffer) > 0:
                        n_step_transition = self.multi_step_buffer._get_n_step_transition()
                        self.memory.add(n_step_transition)
                        self.multi_step_buffer.buffer.popleft()
